...
router.delete('/users/:id', async (req, res) => {
  ...
  try {
    ...
    const userCheck = await query('SELECT id FROM users WHERE id = $1::uuid', [id])
    ...
    const userProducts = await query('SELECT id FROM products WHERE seller_id = $1::uuid', [id])
    const productIds = userProducts.rows.map(p => p.id)

    // Delete orders associated with user's products
    if (productIds.length > 0) {
      await query('DELETE FROM orders WHERE product_id = ANY($1::uuid[])', [productIds])
    }
    ...
    await query('DELETE FROM products WHERE seller_id = $1::uuid', [id])

    // Delete user's shop (this will cascade due to ON DELETE CASCADE)
    await query('DELETE FROM shops WHERE owner_id = $1::uuid', [id])
